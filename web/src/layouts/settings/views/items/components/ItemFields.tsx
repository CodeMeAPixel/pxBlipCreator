  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-center gap-3">
        {SpriteImg && (
          <div className="w-10 h-10 rounded-lg border border-border bg-surface-200 overflow-hidden flex items-center justify-center">
            <img
      ...
      <div className="grid grid-cols-8 gap-1.5">
        {itemData.map((item) => {
          const isSelected = SpriteImg === item.img;
          return (
            <button
              key={item.id}
              onClick={() => handleImageClick(item.img, item.id)}
              className={cn(
                'relative w-full aspect-square rounded-lg bg-surface-200 border transition-all cursor-pointer overflow-hidden flex items-center justify-center',
                'hover:scale-110 hover:z-10 hover:shadow-lg hover:shadow-black/30 hover:border-border-hover',
                isSelected
                  ? 'ring-2 ring-px-500 ring-offset-1 ring-offset-surface-0 scale-105 z-10 border-px-600'
                  : 'border-border'
              )}
            >
              <img
                ...
                loading="lazy"
                className={cn('w-full h-full object-contain', isSelected && 'opacity-50')}
              />
              {isSelected && (
